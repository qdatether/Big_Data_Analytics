function [RM,ecC] = PMIMEsigExact1stLag(allM,Lmax,T,nnei,nsur,alpha,showtxt)
% function [RM,ecC] = PMIMEsigExact1stLag(allM,Lmax,T,nnei,nsur,alpha,showtxt)
% PMIMEsigExact1stLag (Partial Mutual Information on Mixed Embedding, 
% Significance and exact randomization test for the first lag using mutual
% information) computes the measure R_{X->Y|Z} for all combinations of X
% and Y time series from the multivariate time series given in matrix
% 'allM', of size N x K, where Z contains the rest K-2 time series.
% This function is as PMIMEsig but makes an exact randomization test to
...
% one with maximum mutual information to the response. In PMIMEsig the
% randomization is only on the selected lag, which makes it faster but less
% accurate (it favors rejection of H0, i.e. selection of the lagged 
% variable as statistically significant). This tets may be used in time 
% series that appear to be totally random, such as in finance, and then
% this fucntion should be preferred. Otherwise, one should use PMIMEsig. 
